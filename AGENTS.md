...
- Semantic-ish versioning tracking the upstream release the fork is based on: the fork's version is the
  major and minor version of upstream plus a fork revision as the patch version, currently `0.11.3` - the
  third revision on top of upstream `0.11.0` (see `<version>` in `pom.xml`); releases are listed in
  [CHANGELOG.md](CHANGELOG.md), and there is no `publiccode.yml` in this repository. Do not go back to the
  four-part versions of `0.11.0.1` and `0.11.0.2`: not every tool consuming the version can parse them.
